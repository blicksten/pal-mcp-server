...
**Pipeline types:** `feature`, `bugfix`, `deploy`, `audit`, `qa`, `review`, `refactor`, `incident`, `migration`, `spike`, `perf`, `onboard`, `docs`, `techdebt`, `deep-validate`, `testing`, `research`, `compare`, `analysis`.

### Testing Pipeline Routing

**Positive signals** (route to `testing` pipeline):
- "full test suite", "test execution", "coverage audit", "release validation", "generate test map", "run all tests formally"

**Required action chain** when `route_task` returns `testing`:
1. qa-lead calls `generate_test_map_tool` to create the YAML test map
2. qa-lead calls `start_testing_pipeline` to launch the immutable pipeline
3. test-engineer executes each step per locked commands
4. qa-lead verifies baseline check on completion

**Negative signals** (stay with ad-hoc Per-Phase Gate, do NOT route to testing):
- "phase gate", "quick check", "smoke test", "run pytest" (simple ad-hoc execution)

Per-Phase Gate and TEP are independent mechanisms. Per-Phase Gate never starts TEP.
