...
### After These Events, Save Memory:
...
**Note:** MEMORY.md is auto-generated by `memory_ops(action='sync')`. Write the memory file, then call `memory_ops(action='sync', project=<project>)` — do NOT manually edit MEMORY.md.

### Memory Compaction (stale memory cleanup)

With compaction, saving more raw memories is safe — stale ones will be summarized and archived automatically.

**When to compact:** When the memory directory grows beyond ~30 active files, or when memories older than 30 days accumulate and are no longer individually useful.

**Compaction workflow:**
1. Call `memory_ops(action='compact_candidates', project=<project>, staleness_days=30)` — returns stale memories with content
2. Read the candidates, then call PAL MCP (`mcp__pal__chat`) to summarize them into a digest
3. Call `memory_ops(action='compact_execute', project=<project>, source_ids="id1,id2,id3", digest_content="<summary>", digest_name="digest-<topic>")` — creates digest, archives originals, regenerates MEMORY.md

**Canonical promotion:** Some memories should never be compacted (persistent user preferences, critical feedback). Promote them:
- `memory_ops(action='promote', project=<project>, short_id=<id>)` — sets status to `canonical`
- Canonical memories appear in a dedicated `## Canonical` section in MEMORY.md
- Good candidates for canonical: user role/preferences, recurring feedback patterns, long-term project decisions
